...
- **ServeMux method+path patterns (Go 1.22+)**: Register a function on a mux with a method and pattern; wildcards come from `r.PathValue`.
  - When to use: Wiring HTTP routes in new code.
  - How: `mux := http.NewServeMux(); mux.HandleFunc("GET /{$}", QR); mux.HandleFunc("GET /item/{id}", item)` then `http.ListenAndServe(*addr, mux)`. `QR` still has signature `func(http.ResponseWriter, *http.Request)` — `HandlerFunc` is still the adapter (Ch 12).
  - History: Effective Go registered `http.Handle("/", http.HandlerFunc(QR))` on the default mux. Keep that as the document's example, not the 1.22+ default. See [modern.md](../references/modern.md).
...
## Mental Models
- Think of a Go web server as: parse flags -> `http.NewServeMux()` -> `HandleFunc("METHOD /path/{id}", f)` -> `ListenAndServe(addr, mux)` -> `log.Fatal` on error. That's the whole skeleton.
...
- **Building handlers as heavyweight structs when a function + `HandleFunc` suffices** (Ch 12).
- **Defaulting to `http.Handle("/", …)` on Go 1.22+**: use method-aware patterns on an explicit mux.
...
```
- **What it demonstrates**: the 1.22+ Go HTTP server skeleton - flags, method-aware ServeMux patterns, `ListenAndServe` with an explicit mux, and fatal error handling. Effective Go's original `http.Handle("/", http.HandlerFunc(QR))` is the same idea on the 2009 default mux.

## Worked Example
The whole program is the worked example: a "web re-server" that takes a short piece of text, calls Google's chart service to render a QR code, and returns the image. The flow: `main` parses `-addr`, registers `QR` at `GET /{$}` on an explicit mux, and serves. (The 2009 text used `http.Handle("/", http.HandlerFunc(QR))` on the default mux.) `QR` reads the request, builds the chart-API URL with the user's text, and uses `templ` (parsed once at startup via `template.Must`) to render an HTML page that embeds that URL - so a phone's camera can read the resulting QR code as a URL. The program exercises nearly every prior chapter: formatting/naming conventions in its layout, `flag` and `init`-style setup, `io.Writer` via `ResponseWriter`, the `HandlerFunc` interface adapter, and `log.Fatal` error handling. Lesson: a real Go server is short because the idioms compose.

## Key Takeaways
1. Register routes with `mux.HandleFunc("GET /path/{id}", f)` (Go 1.22+); `HandlerFunc` is still how a function becomes an `http.Handler`.
